...
func run(context) -> void:
	...
	var clockHoldTicksControl := main.get("ClockHoldTicksControl") as Button
	...
	assert(clockHoldTicksControl.text == "1")
	assert(clockHoldTicksControl.mouse_default_cursor_shape == Control.CURSOR_HSIZE)
	assert(String(board.get("SelectedTool")) == selectedToolBefore)
	assert(int(board.get("CurrentInteractionMode")) == interactionModeBefore)

	main.call("handleClockHoldTicksInput", context.makeMouseButtonEvent(clockHoldTicksControl, MOUSE_BUTTON_LEFT, true))
	var increaseEvent: InputEventMouseMotion = context.makeMouseMotionEvent(clockHoldTicksControl.get_global_rect().get_center(), MOUSE_BUTTON_MASK_LEFT)
	increaseEvent.relative = Vector2(16.0, 0.0)
	main.call("handleClockHoldTicksInput", increaseEvent)
	main.call("handleClockHoldTicksInput", context.makeMouseButtonEvent(clockHoldTicksControl, MOUSE_BUTTON_LEFT, false))
	await context.waitFrames(1)
	assert(clockHoldTicksControl.text == "3")
	assert(int(board.call("getClockHoldTicks")) == 3)

	main.call("handleClockHoldTicksInput", context.makeMouseButtonEvent(clockHoldTicksControl, MOUSE_BUTTON_LEFT, true))
	var decreaseEvent: InputEventMouseMotion = context.makeMouseMotionEvent(clockHoldTicksControl.get_global_rect().get_center(), MOUSE_BUTTON_MASK_LEFT)
	decreaseEvent.relative = Vector2(-256.0, 0.0)
	main.call("handleClockHoldTicksInput", decreaseEvent)
	main.call("handleClockHoldTicksInput", context.makeMouseButtonEvent(clockHoldTicksControl, MOUSE_BUTTON_LEFT, false))
	await context.waitFrames(1)
	assert(clockHoldTicksControl.text == "1")
	...
	assert(int((clipboardTiles[0] as Dictionary).get("clockHoldTicks", 0)) == 3)
	var pastedCoordinates := Vector2i(-8, -8)
	var pastedTiles: Dictionary = board.call("getPasteTileMap", pastedCoordinates, clipboardItem)
	assert(int((pastedTiles[pastedCoordinates] as Dictionary).get("clockHoldTicks", 0)) == 3)
