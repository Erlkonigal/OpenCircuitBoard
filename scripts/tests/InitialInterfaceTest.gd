...
func run(context) -> void:
	...
	var loopFrequencyMaximumTps := float(main.call("getLoopFrequencyMaximumTps"))
	assert(loopFrequencyMaximumTps >= 1.0)
	assert(is_equal_approx(loopFrequencySlider.min_value, 0.0))
	assert(is_equal_approx(loopFrequencySlider.max_value, loopFrequencyMaximumTps))
	...
	assert(float(main.get("LoopFrequency")) <= loopFrequencyMaximumTps)
